 ...
 */
public class AssistantNurseAgent extends Agent {
    ...
    protected void setup() {
        ...
        try {
            ...
            clips.build("""
                    (defrule assistant-nurse-operate
                        (operating-room)
                      =>
            ...
            clips.build("""
                    (defrule nurse-provides-material
                       (material-requested)
                       =>
                       (printout t "{'to':'surgeon-2-agent-1','knowledge':'material-provided','legend':'Enfermera asistente: Aquí tienes el material e instrumentos necesarios.'}" crlf)
                    )""");
            ...
            clips.build("""
                    (defrule nurse-takes-patient-recovery-room
                      ...
                      =>
                      (printout t "{'to':'patient-agent-1','knowledge':'recuperation-room','legend':'Enfermera asistente: llevo al paciente a la sala de recuperación.'}" crlf)
                    )""");
        ...
        cfp.setContent(
                new MsgContent(null,"assistant-nurse-ready", "ready").toJsonString()
        );

        addBehaviour(new ContractNetInitiator(this, cfp) {
            protected void handlePropose(ACLMessage propose, Vector acceptances) {
                // Procesar la propuesta recibida
                System.out.println("Enfermera asistente: Propuesta recibida de " + propose.getSender().getName());
