...
public class ChiefSurgeonAgent extends Agent {
    ...
    protected void setup() {
        ...
        try {
            ...
            clips.build("""
                    (defrule chief-surgeon-orders-anesthesiologist
                       (operation-started)
                       =>
                       (printout t "{'to':'anesthesiologist-agent-1','knowledge':'anesthetic-calculated', 'legend':'Cirujano en Jefe: Anestesiólogo, confirma el cálculo del anestésico y aplícalo.'}" crlf)
                     )""");
            ...
            clips.build("""
                    (defrule chief-surgeon-orders-surgeon2
                      (patient-sedated)
                      =>
                      (printout t "{'to':'surgeon-2-agent-1','knowledge':'surgeon2-starts-intervention', 'legend':'Cirujano en Jefe: Cirujano 2, puedes comenzar la intervención.'}" crlf)
                    )
                    """);
        ...
        cfp.setContent(
                new MsgContent(null, "chief-surgeon-ready", "ready").toJsonString()
        );

        addBehaviour(new ContractNetInitiator(this, cfp) {
            protected void handlePropose(ACLMessage propose, Vector acceptances) {
                // Procesar la propuesta recibida
                System.out.println("Cirujano Jefe: Propuesta recibida de " + propose.getSender().getName());
