...
public class PatientAgent extends Agent {
    ...
    protected void setup() {
        ...
        try {
            ...
            clips.build("""
                    (defrule patient-recuperate
                        (recuperation-room)
                      =>
                      (printout t "{'to':'','knowledge':null,'legend':'Paciente: Estoy en la sala de recuperación.'}" crlf)
            ...
            clips.build("""
                    (defrule end
                      (patient-recovery)
                      =>
                      (printout t "{'to':'','knowledge':null,'legend':'Intervención quirúrgica finalizada.'}" crlf)
                    )
                    """);
        ...
        cfp.setContent(
                new MsgContent(null, "patient-ready", "ready").toJsonString()
        );
