 ...
 */
public class Surgeon2Agent extends Agent {
    ...
    protected void setup() {
        ...
        try {
            ...
            clips.build("""
                    (defrule surgeon2-requests-material
                      ...
                      =>
                      (printout t "{'to':'assistant-nurse-agent-1','knowledge':'material-requested', 'legend':'Cirujano 2: Enfermera asistente, por favor, provee el material e instrumentos necesarios.'}" crlf)
                    )""");
            ...
            clips.build("""
                    (defrule surgeon2-finishes-intervention
                      (material-provided)
                      =>
                      (printout t "{'to':'assistant-nurse-agent-1','knowledge':'intervention-completed', 'legend':'Cirujano 2: He finalizado la intervención.'}" crlf)
                    )
                    """);
        ...
        cfp.setContent(
                new MsgContent(null, "surgeon2-ready", "ready").toJsonString()
        );

        addBehaviour(new ContractNetInitiator(this, cfp) {
            protected void handlePropose(ACLMessage propose, Vector acceptances) {
                // Procesar la propuesta recibida
                System.out.println("Paciente: Propuesta recibida de " + propose.getSender().getName());
